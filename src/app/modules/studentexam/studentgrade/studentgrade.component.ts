...
export class StudentgradeComponent implements OnInit {
  ...
  UpdateOrSave(row) {
    ...
    //debugger;
    this.loading = true;
    let checkFilterString = "GradeName eq '" + row.GradeName + "' and OrgId eq " + this.LoginUserDetail[0]["orgId"] + " and BatchId eq " + this.SelectedBatchId;
    ...
    this.dataservice.get(list)
      .subscribe((data: any) => {
        ...
        if (data.value.length > 0) {
          ...
          this.StudentGradeData.ClassId = row.ClassId;
          this.StudentGradeData.GradeTypeId = row.GradeTypeId;
          this.StudentGradeData.BatchId = row.BatchId;
